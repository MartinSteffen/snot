\section{Model Checker (optional)}
\label{sec:assert}


\team{}

Es soll die Möglichkeit gegeben werden, die \emph{Korrektheit} des
eingegebenen SFC's zu überprüfen. Dies soll in einfacher Weise dadurch
geschehen, daß überprüft wird, ob auf allen Ausführungen des Programmes die
\emph{Assertions} nicht verletzt werden.



\medskip

Im wesentlichen wird eine \emph{Graphsuche} implementiert: die
abstrakte Syntax wird in einem ersten Schritt in einen (expliziten oder
impliziten) Graphen übersetzt, der danach mittels \emph{Tiefensuche} nach
Verletzung der Zusicherungen abgesucht wird.


Die Semantik der Sprache ist in Anhang~\ref{sec:semantik} informell
beschrieben.


\subsubsection*{Schnittstelle}

Im wesentlichen mit der Gui (Abschnitt~\ref{sec:gui}): 

Methode \texttt{start\_modcheck} mit Parameter eines SFC's in abstrakter
Syntax. Rückgabe: \emph{noch zu klären}: entweder mittels Ausnahme oder
boolescher Wert. Auf jeden Fall wird der Gui der Zustand zurückgegeben, in
welchem die Verletzung auftritt. Wie die Gui darauf reagiert, ist nicht
Sache des Modelcheckerpaketes (z.B.\ könnte der Zustand gehighlighted
werden).


%%% Local Variables: 
%%% mode: latex
%%% TeX-master: "main"
%%% End: 
